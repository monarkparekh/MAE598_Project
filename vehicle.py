...
class Vehicle(object):
    ...
    def __init__(self, x,y, behavior, window):
        ...
        self.velocity = vec2(0.1,0) # Initial speed
        ...
        self.rotation = atan2(self.location.y, self.location.x) # initial rotation
    ...
    def update(self):
        ...
        # Constrains position to the limits of screen
    ...
    def seek(self, target):
        ...
        # Applies steering force to the drone
    ...
    def arrive(self, target):
        ...
        try:
            dist = copy.deepcopy(self.desired.normalize())
        except: # If the magnitude of desired is zero it cant be normalized
            dist = copy.deepcopy(self.desired)
        
        r = RADIUS_TARGET
        # Modulates the force
        if d < r: # close to target it will reduce velocity till stops
            # interpolation
    ...
    def stay_at(self, center, r = RADIUS_TARGET):
        ...
        if posToCenter.length() > r :
            ...
            print(f'Distance at perimeter: {posToPerimeter.length()}')
            ...
            if self.debug == True:
                pg.draw.line(self.window,(0,255,0), center,  new_target ,5)
    ...
    def align_direction_with_swarm(self, all_positions, index):
        ...
        # aux != index -> avoids the auto-collision check
            ...
            if ( (d > 0) and (d < AVOID_DISTANCE*separation_factor) and (aux != index) ) :
                ...
                diff = diff/d # proportional to the distance. The closer the stronger needs to be
                soma += diff
